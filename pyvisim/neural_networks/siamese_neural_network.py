...
class SiameseNeuralNetwork(torch.nn.Module, SimilarityMetric):
    def __init__(self, backbone: torch.nn.Module, embedding_dim: int,
                 similarity_func: Callable[[MatLike, MatLike],
                                           MatLike] = cosine_similarity,
    ...
    def preprocess(self, image, dims:str="HWC", value_range: tuple[float, float] = (0, 255)) -> torch.Tensor:
        """
    ...
    @torch.no_grad()
    def encode(self, image: MatLike, dims: str = "HWC", value_range: tuple[float, float] = (0, 255)):
        # For encoding image, we need to switch into eval mode so that BatchNorm/ Dropout hevaes correctly during inference
        # Also we're restoring previous training state afterwards so that calling encode() mind-training dosen't break the training loop
    ...
    def similarity_score(self, image_a, image_b, dims_a= "HWC", dims_b = "HWC", value_range: tuple[float, float] = (0, 255)):
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, emb_a: torch.Tensor, emb_b: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        loss = 0.5 * (labels * distance.pow(2) + (1 - labels)
